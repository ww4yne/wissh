...
usage() {
  cat >&2 <<'USAGE'
Usage:
  scripts/wissh_live_ui_test_with_cleanup.sh [--configuration Debug|Release] [--development-team <team-id>] [--derived-data-path <path>] --only-testing <test-id> [--only-testing <test-id> ...]
  scripts/wissh_live_ui_test_with_cleanup.sh --dry-run-cleanup <manifest-file>

Runs selected Wissh live SSH UI tests using /tmp/remux-live-ssh.json and
...
if declare -p WISSH_PROFILE_PANE_SWITCH_COUNT >/dev/null 2>&1; then
  value="$WISSH_PROFILE_PANE_SWITCH_COUNT"
  if [[ ! "$value" =~ ^[0-9]+$ ]] || (( 10#$value < 1 || 10#$value > 1000 )); then
    printf 'WISSH_PROFILE_PANE_SWITCH_COUNT must be an integer from 1 through 1000; got %s.\n' "$value" >&2
    exit 1
  fi
fi
for name in WISSH_TRACE_FLOWS WISSH_TRACE_TMUX_VIEWPORT WISSH_TRACE_LATENCY WISSH_TRACE_PERF GHOSTTY_TRACE_SURFACE_INIT GHOSTTY_TRACE_FRAME_COMPLETION; do
...
if [[ -z "$known_host_line" ]]; then
  printf 'No trusted OpenSSH host key found for %s; refusing automated Wissh trust.\n' "$known_host_lookup" >&2
  exit 2
fi
...
printf 'pid=%s\nstartedAt=%s\n' "$$" "$(date +%s)" >"$harness_file"
for name in \
  WISSH_LIVE_AGENT_TUI_SESSION \
  WISSH_PROFILE_PANE_SWITCH_COUNT \
  WISSH_TRACE_FLOWS \
...
prepare_dense_mixed_fixture() {
  ...
  WISSH_LIVE_SSH_SECRET="$ssh_askpass_secret" \
    SSH_ASKPASS="$askpass" \
...
"$tmux_bin" send-keys -t "$pane4" "printf 'WISSH_DENSE_MIXED_READY_P4\n'" C-m
...
WISSH_PREVIEW_FILE_CONTENT_ALPHA
WISSH_PREVIEW_FILE_CONTENT_BETA
WISSH_PREVIEW_FILE_CONTENT_GAMMA
PREVIEW_FILE
...
cleanup_generated_sessions() {
  ...
  while IFS= read -r session; do
    ...
    if ! WISSH_LIVE_SSH_SECRET="$ssh_askpass_secret" \
      SSH_ASKPASS="$askpass" \
      SSH_ASKPASS_REQUIRE=force \
      DISPLAY=remux \
      ssh \
  ...
  while IFS=$'\t' read -r kind session arg1 arg2 extra || [[ -n "${kind:-}" ]]; do
    ...
    case "$kind" in
      window-count)
        ...
        local actual
        if ! actual="$(WISSH_LIVE_SSH_SECRET="$ssh_askpass_secret" \
          SSH_ASKPASS="$askpass" \
          SSH_ASKPASS_REQUIRE=force \
          DISPLAY=remux \
          ssh \
...
if [[ "$configuration" == "Release" ]]; then
  xcode_args+=('SWIFT_ACTIVE_COMPILATION_CONDITIONS=$(inherited) WISSH_LIVE_UI_TESTING')
fi
...
set +e
WISSH_TRACE_LATENCY="${WISSH_TRACE_LATENCY:-1}" \
WISSH_TRACE_PERF="${WISSH_TRACE_PERF:-1}" \
GHOSTTY_TRACE_SURFACE_INIT="${GHOSTTY_TRACE_SURFACE_INIT:-1}" \
WISSH_LIVE_GENERATED_SESSION_MANIFEST="$manifest" \
WISSH_LIVE_TMUX_EXPECTATION_MANIFEST="$expectations" \
WISSH_LIVE_PREPARED_FIXTURE="$fixture_name" \
WISSH_LIVE_SESSION_NAME_OVERRIDE="$fixture_session" \
WISSH_LIVE_EXPECTED_HOST_KEY="$expected_host_key" \
WISSH_LIVE_SSH_CONFIGURATION_BASE64="$live_ssh_configuration_base64" \
