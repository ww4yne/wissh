...
enum GhosttyScrollTuning {
    ...
    /// mouse-report route. `WISSH_SCROLL_PRECISE_GAIN` overrides for
    /// on-device feel experiments (clamped; same read-once pattern as
    /// the WISSH_TRACE_* flags).
    static let routeForwardedGain: CGFloat = {
        let fallback = routeForwardedDefaultGain
        guard
            let raw = ProcessInfo.processInfo.environment["WISSH_SCROLL_PRECISE_GAIN"],
            let value = Double(raw), value.isFinite
        else {
            return fallback
        }
        let clamped = CGFloat(min(max(value, 0.5), 4))
        NSLog("Wissh scrollTuning preciseGain=%.2f (env override)", clamped)
        return clamped
    }()
}
