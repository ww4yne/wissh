...
struct GhosttyDebugComposerDictationConfiguration: Sendable {
    ...
    static func current(
        ...
        if environment["WISSH_DEBUG_DICTATION_NO_SPEECH"] == "1" {
            return GhosttyDebugComposerDictationConfiguration(transcript: nil)
        }
        guard let transcript = environment["WISSH_DEBUG_DICTATION_TRANSCRIPT"],
    ...
    @unchecked Sendable
{
    ...
    private let queue = DispatchQueue(label: "app.wissh.composer-dictation-debug")
