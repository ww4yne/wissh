...
enum KeychainSSHCredentialStoreError: LocalizedError, Sendable {
    ...
    var errorDescription: String? {
        ...
        case .loadFailed(let status):
            return "Wissh could not load the saved SSH credential from Keychain (\(status))."
        case .saveFailed(let status):
            return "Wissh could not save the SSH credential in Keychain (\(status))."
        case .deleteFailed(let status):
            return "Wissh could not delete the saved SSH credential from Keychain (\(status))."
        case .invalidPayload:
            return "Wissh received an invalid Keychain SSH credential payload."
        }
    }
}

actor KeychainSSHCredentialStore: SSHCredentialStore {
    static let defaultService = "app.wissh.ssh-credentials"
