...
enum TrustedHostStoreError: LocalizedError, Sendable {
    ...
    var errorDescription: String? {
        ...
        case .hostKeyTrustRequired(let challenge):
            ...
            case .unknown:
                return "The SSH host key for \(challenge.host) is unknown. Wissh refused the connection until you trust it."
            case .changed:
                return "The SSH host key for \(challenge.host) changed. Wissh refused the connection."
            }
        case .staleHostKeyTrust(let host):
            return "The SSH host key trust state for \(host) changed. Try connecting again."
        case .invalidHostKey:
            return "Wissh could not read the SSH host key."
        }
    }
}
...
final class TrustedHostStore: @unchecked Sendable {
    ...
    func validator(for proxyJump: ProxyJumpConfiguration) -> SSHHostKeyValidator {
        SSHHostKeyValidator.custom(
            StoredHostKeyValidator(
                connectionID: proxyJump.id,
                host: proxyJump.host,
                store: self
            )
        )
    ...
    fileprivate func validate(
        ...
        try lock.withLock {
            ...
            if let existing = identities.first(where: { $0.serverID == connectionID }) {
...
private final class StoredHostKeyValidator: NIOSSHClientServerAuthenticationDelegate, @unchecked Sendable {
    ...
    init(connectionID: UUID, host: String, store: TrustedHostStore) {
