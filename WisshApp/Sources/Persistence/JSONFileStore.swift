...
enum JSONFileStoreError: LocalizedError, Sendable {
    ...
    var errorDescription: String? {
        ...
        case .invalidDirectory(let url):
            return "Wissh could not create \(url.path)."
        case .readFailed(let url, let message):
            return "Wissh could not read \(url.lastPathComponent): \(message)"
        case .decodeFailed(let url, let message):
            return "Wissh could not decode \(url.lastPathComponent): \(message)"
        case .encodeFailed(let url, let message):
            return "Wissh could not encode \(url.lastPathComponent): \(message)"
        case .writeFailed(let url, let message):
            return "Wissh could not write \(url.lastPathComponent): \(message)"
        }
    }
}
