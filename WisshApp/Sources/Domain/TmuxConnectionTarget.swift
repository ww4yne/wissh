...
struct ResolvedSSHAuth: Equatable, Sendable {
    ...
    func using(username: String) -> ResolvedSSHAuth {
        ResolvedSSHAuth(
            identityID: identityID,
            username: username,
            displayLabel: displayLabel,
...
struct ProxyJumpConfiguration: Identifiable, Equatable, Codable, Sendable {
    let id: UUID
    var host: String
    var port: Int
    var username: String
...
struct TmuxConnectionTarget: Equatable, Sendable {
    ...
    let proxyJumpSSHAuth: ResolvedSSHAuth?
    ...
    init(
        ...
        proxyJumpSSHAuth: ResolvedSSHAuth? = nil,
        terminalSettings: TerminalSettings = .default
    ) {
        self.server = server
        self.workspace = workspace
        self.sshAuth = sshAuth
        if let proxyJump = server.proxyJump {
            self.proxyJumpSSHAuth = proxyJumpSSHAuth ?? (
                proxyJump.identityID == sshAuth.identityID
                    ? sshAuth.using(username: proxyJump.username)
                    : nil
            )
        } else {
            self.proxyJumpSSHAuth = nil
        }
...
struct TmuxConnectionDraft: Equatable, Sendable {
    let serverID: SavedServer.ID
    let proxyJumpID: ProxyJumpConfiguration.ID
    var displayName: String = ""
    var host: String = ""
    var port: String = "22"
    var username: String = ""
    ...
    init(
        ...
        proxyJumpID: ProxyJumpConfiguration.ID = UUID()
    ) {
        self.serverID = serverID
    ...
    init(server: SavedServer, workspace: SavedWorkspace) {
        self.init(
            serverID: server.id,
            proxyJumpID: server.proxyJump?.id ?? UUID()
        )
        ...
        if let proxyJump = server.proxyJump {
            self.usesProxyJump = true
            self.proxyJumpHost = proxyJump.host
            self.proxyJumpPort = String(proxyJump.port)
            self.proxyJumpUsername = proxyJump.username
            self.proxyJumpIdentityID = proxyJump.identityID == server.identityID
                ? nil
        ...
        SavedServer(
            ...
            proxyJump: proxyJump.map {
                ProxyJumpConfiguration(
                    ...
                    port: $0.port,
                    username: $0.username,
                    identityID: $0.identityID ?? identityID
                )
            }
        )
    }
...
struct ValidatedProxyJumpDraft: Equatable, Sendable {
    let id: ProxyJumpConfiguration.ID
    let host: String
    let port: Int
    let username: String
    let identityID: SSHIdentity.ID?
}
...
enum TmuxConnectionDraftValidator {
    ...
    static func validateServer(
        ...
        let proxyJumpHost = draft.proxyJumpHost.trimmingCharacters(in: .whitespacesAndNewlines)
        let proxyJumpUsername = draft.proxyJumpUsername.trimmingCharacters(in: .whitespacesAndNewlines)
        ...
        var proxyJump: ValidatedProxyJumpDraft?
        if draft.usesProxyJump {
            if proxyJumpHost.isEmpty {
                validation.proxyJumpHost = "ProxyJump HostName is required."
            }
            if proxyJumpUsername.isEmpty {
                validation.proxyJumpUsername = "ProxyJump User is required."
            }
            guard let proxyJumpPort = Int(draft.proxyJumpPort),
                  (1...65_535).contains(proxyJumpPort) else {
                validation.proxyJumpPort = "ProxyJump Port must be between 1 and 65535."
                return .invalid(validation)
            }
            proxyJump = ValidatedProxyJumpDraft(
                id: draft.proxyJumpID,
                host: proxyJumpHost,
                port: proxyJumpPort,
                username: proxyJumpUsername,
                identityID: draft.proxyJumpIdentityID
            )
        }
...
private extension TmuxConnectionDraftValidation {
    mutating func merge(_ other: TmuxConnectionDraftValidation) {
        ...
        proxyJumpHost = proxyJumpHost ?? other.proxyJumpHost
        proxyJumpPort = proxyJumpPort ?? other.proxyJumpPort
        proxyJumpUsername = proxyJumpUsername ?? other.proxyJumpUsername
