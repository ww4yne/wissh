    ...
    func schedule(
        ...
        task = Task.detached(priority: .utility) { [weak self] in
            ...
            for candidate in candidates {
                ...
                guard candidate.server.proxyJump?.identityID == nil ||
                        candidate.server.proxyJump?.identityID == candidate.server.identityID else {
                    await MainActor.run {
                        self?.traceSkipped(
                            candidate: candidate,
                ...
                do {
                    ...
                    NSLog(
                        "Wissh library SSH prewarm failed for %@: %@",
