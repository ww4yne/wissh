...
private struct RemuxWorkspaceShell: View {
    ...
    private func connectionSetupView(
        ...
        return ConnectionSetupView(
            ...
            availableIdentities: model.library.identities,
    ...
    private var libraryStack: some View {
        ...
        .alert("Server Order Couldn’t Be Saved", isPresented: Binding(
            ...
            Text("Your order is still shown, but may be lost when you close Wissh. Try saving it again.")
        }
    }
    ...
    func body(content: Content) -> some View {
        content.alert(
            ...
            Text(
                ...
                "Complete it in your browser, then return to Wissh."
            )
        }
        ...
        List {
            ...
            Section("Sessions") {
                if workspaces.isEmpty {
                    Text("No Sessions Added to Wissh")
                        .foregroundStyle(.secondary)
                        .padding(.vertical, 2)
                        .frame(maxWidth: .infinity, alignment: .leading)
    ...
    private var availableSessionsSection: some View {
        Section {
            ...
            if !availableSessionNames.isEmpty {
                Text("Select a session to add it to Wissh.")
            }
        }
        .libraryHomeListRowSurface()
    }
...
private func sshHostKeyTrustMessage(for challenge: SSHHostKeyTrustChallenge) -> String {
    guard let fingerprint = challenge.receivedKeyFingerprint else {
        return "Wissh couldn’t verify the SSH host key for \(challenge.host), so trust is unavailable."
    }
...
private struct AvailableServerSessionRow: View {
    ...
    var body: some View {
        ...
        .accessibilityHint("Add this session to Wissh")
    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                Text(
                    "You can override this per window from Panes. Wissh normally clears zooms "
            ...
            Section {
                ...
                Text(
                    "For servers that only support ssh-rsa (SHA-1). Enabling takes effect "
                        + "immediately; disabling takes effect after restarting Wissh."
                )
            }
            .libraryHomeListRowSurface()
        }
...
struct ConnectionSetupView: View {
    ...
    let setupSessionID: UUID?
    let terminalTheme: TerminalTheme
    let availableIdentities: [SSHIdentity]
                ...
                Section {
                    ...
                    if draft.usesProxyJump {
                        textInputRow(
                            ...
                            textStyle: .monospaced,
                            keyboardType: .URL,
                            accessibilityIdentifier: "connection.proxy-jump.host"
                        )

                        textInputRow(
                            title: "Port",
                            placeholder: "22",
                            keyPath: \.proxyJumpPort,
                            field: .proxyJumpPort,
                            validationMessage: validation.proxyJumpPort,
                            textStyle: .monospaced,
                            keyboardType: .numberPad,
                            accessibilityIdentifier: "connection.proxy-jump.port"
                        )
                        ...
                        textInputRow(
                            ...
                            textStyle: .monospaced,
                            accessibilityIdentifier: "connection.proxy-jump.username"
                        )

                        Picker("IdentityFile", selection: proxyJumpIdentityBinding) {
                            Text("Same as target").tag(SSHIdentity.ID?.none)
                    ...
                    Text(
                        "Uses standard OpenSSH ProxyJump semantics. " +
                        "The private key remains on this iPhone."
                    )
                }
                .connectionSetupListRowSurface(usesLibraryChrome: showsEditableServerFields)
            }
    ...
    private func nextField(after field: Field) -> Field? {
        ...
        case .port:
            return .username
        case .username:
            if draft.usesProxyJump { return .proxyJumpHost }
            if showsAuthenticationFields {
                switch draft.authenticationKind {
                case .password:
                    return .password
            ...
            return nil
        case .proxyJumpHost:
            return .proxyJumpPort
        case .proxyJumpPort:
            return .proxyJumpUsername
        case .proxyJumpUsername:
            if showsAuthenticationFields {
                switch draft.authenticationKind {
                case .password:
    ...
    private var proxyJumpBinding: Binding<Bool> {
        Binding(
            get: { draft.usesProxyJump },
            set: { enabled in
                onChange { draft in
                    draft.usesProxyJump = enabled
                }
            }
        )
    }

    private var proxyJumpIdentityBinding: Binding<SSHIdentity.ID?> {
        Binding(
            get: { draft.proxyJumpIdentityID },
            set: { identityID in
                onChange { draft in
                    draft.proxyJumpIdentityID = identityID
                }
            }
        )
    }
