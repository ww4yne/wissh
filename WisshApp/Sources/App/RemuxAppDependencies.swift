...
struct RemuxAppDependencies: Sendable {
    ...
    static func launch() -> Result<RemuxAppDependencies, Error> {
        Result {
            ...
            if ProcessInfo.processInfo.environment["WISSH_UI_TESTING"] == "1" {
        ...
        if usesEphemeralDebugStorage {
            root = try ApplicationStorage.remuxRoot(
                overridePath: FileManager.default.temporaryDirectory
                    .appendingPathComponent("WisshLiveDebug-\(UUID().uuidString)", isDirectory: true)
                    .path
            )
    ...
    private static func jumpConfiguration(
        for target: TmuxConnectionTarget,
        trustedHostStore: TrustedHostStore,
        connectTimeout: TimeAmount
    ) -> OpenSSHProxyJumpConfiguration? {
        guard let proxyJump = target.server.proxyJump else { return nil }

        return OpenSSHProxyJumpConfiguration(
            host: proxyJump.host,
            port: proxyJump.port,
            authenticationMethod: {
                guard let jumpAuth = target.proxyJumpSSHAuth else {
                    throw SSHAuthResolverError.missingIdentity(proxyJump.identityID)
                }
                return try SSHAuthenticationMethodFactory.make(
                    username: jumpAuth.username,
                    credential: jumpAuth.credential
                )
            },
            hostKeyValidator: trustedHostStore.validator(for: proxyJump),
            connectTimeout: connectTimeout,
            authenticationTimeout: connectTimeout
        )
    }
    ...
    private enum DebugLiveEnvironmentKey {
        static let ephemeralStorage = "WISSH_DEBUG_EPHEMERAL_STORAGE"
    }
    ...
    static func uiTesting() throws -> RemuxAppDependencies {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("WisshUITesting", isDirectory: true)
        let trustedHostStore = TrustedHostStore(rootURL: root)
        let publicKeyInstallOutcome = ProcessInfo.processInfo.environment[
            "WISSH_UI_TEST_PUBLIC_KEY_INSTALL_OUTCOME"
        ...
        let tailscaleSSHCheckChallenge = ProcessInfo.processInfo.environment[
            "WISSH_UI_TEST_TAILSCALE_CHECK_BANNER"
    ...
    private static func uiTestingTransportChunks() -> [Data] {
        guard ProcessInfo.processInfo.environment["WISSH_UI_TEST_INPUT_READY"] == "1" else {
            return []
        }
