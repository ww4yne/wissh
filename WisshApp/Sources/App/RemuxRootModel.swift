...
final class RemuxRootModel: ObservableObject {
    ...
    func cancelSetup() {
        ...
        case .newServer:
            ...
            do {
                ...
                NSLog(
                    "Wissh provisional trusted-host cleanup failed for server %@: %@",
    ...
    private func saveNewServer(
        ...
        case .valid(let submission):
            ...
            do {
                let targetSSHAuth = try resolvedSSHAuth(
                    from: submission,
                    identityCredential: identityCredential
                )
                let sshAuth = try await resolvedConnectionAuth(
                    for: server,
                    targetSSHAuth: targetSSHAuth
                )
        ...
        do {
            ...
            if !savedServer && savedIdentity {
                do {
                    ...
                    NSLog(
                        "Wissh new-server identity cleanup failed: %@",
                        String(describing: error)
                    )
                }
            }
            if !savedServer {
                ...
                guard isCurrentSetupAction(action) else { return nil }
                NSLog(
                    "Wissh new-server persistence failed before server save: %@",
                    String(describing: error)
                )
    ...
    private func failEditServerSave(
        ...
        do {
            ...
            NSLog(
                "Wissh trusted-host restore failed: %@",
    ...
    private func connect(
        ...
        let sshAuth: ResolvedConnectionAuth
        do {
    ...
    func updateTerminalSettings(_ mutation: (inout TerminalSettings) -> Void) async {
        ...
        do {
            ...
            NSLog("Wissh terminal settings save failed: %@", String(describing: error))
    ...
    private func applyTerminalSettingsToActiveSessions(_ settings: TerminalSettings) {
        ...
        for session in activeSessions {
            ...
            do {
                ...
                NSLog("Wissh terminal settings apply failed: %@", String(describing: error))
            }
        }
    }
    ...
    private func activate(
        server: SavedServer,
        workspace: SavedWorkspace,
        sshAuth: ResolvedConnectionAuth
    ) {
    ...
    private struct ResolvedConnectionAuth {
        let target: ResolvedSSHAuth
        let proxyJump: ResolvedSSHAuth?
    }

    private func resolveSSHAuth(for server: SavedServer) async throws -> ResolvedConnectionAuth {
        let resolver = SSHAuthResolver(
            credentialStore: dependencies.credentialStore
        )
        let target = try await resolver.resolve(server: server, in: library)
        return try await resolvedConnectionAuth(
            for: server,
            targetSSHAuth: target,
            resolver: resolver
        )
    }

    private func resolvedConnectionAuth(
        for server: SavedServer,
        targetSSHAuth: ResolvedSSHAuth,
        resolver: SSHAuthResolver? = nil
    ) async throws -> ResolvedConnectionAuth {
        guard let proxyJump = server.proxyJump else {
            return ResolvedConnectionAuth(target: targetSSHAuth, proxyJump: nil)
        }
        if proxyJump.identityID == targetSSHAuth.identityID {
            return ResolvedConnectionAuth(
                target: targetSSHAuth,
                proxyJump: targetSSHAuth.using(username: proxyJump.username)
            )
        }
        let resolver = resolver ?? SSHAuthResolver(
            credentialStore: dependencies.credentialStore
        )
        let jumpAuth = try await resolver.resolve(
            identityID: proxyJump.identityID,
            username: proxyJump.username,
            in: library
        )
        return ResolvedConnectionAuth(target: targetSSHAuth, proxyJump: jumpAuth)
    }
    ...
    private static func newServerVerificationMessage(for error: any Error) -> String {
        ...
        case .serverUnreachable:
            return "Wissh could not reach the server. Check the address, port, network, or VPN."
        case .authentication:
            return "The server rejected these credentials. Check the user and authentication details."
        case .tmuxUnavailable:
            return reason.message
        case .hostKey:
            return "Wissh could not verify the SSH host key."
    ...
    private func cleanupCreatedCredential(
        ...
        if savedCredential {
            do {
                ...
                NSLog(
                    "Wissh SSH credential cleanup failed: %@",
                    String(describing: error)
                )
            }
        }
    ...
    private func restoreCredential(
        ...
        do {
            ...
            NSLog(
                "Wissh SSH credential restore failed: %@",
                String(describing: error)
            )
        }
    }
    ...
    private func restoreIdentity(_ identity: SSHIdentity) async {
        do {
            ...
            NSLog(
                "Wissh SSH identity restore failed: %@",
                String(describing: error)
            )
        }
    }
    ...
    private func target(
        ...
        TmuxConnectionTarget(
            server: server,
            workspace: workspace,
            sshAuth: sshAuth.target,
            proxyJumpSSHAuth: sshAuth.proxyJump,
