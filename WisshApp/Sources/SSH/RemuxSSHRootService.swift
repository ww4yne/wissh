...
struct RemuxSSHRootKey: Hashable, Sendable {
    ...
    private let proxyJump: String?
    private let jumpPort: Int?
    private let jumpUsername: String?
    private let jumpAuthFingerprint: String?
    ...
    init(target: TmuxConnectionTarget) {
        ...
        self.proxyJump = target.server.proxyJump?.host
        self.jumpPort = target.server.proxyJump?.port
        self.jumpUsername = target.proxyJumpSSHAuth?.username
        self.jumpAuthFingerprint = target.proxyJumpSSHAuth?.authFingerprint
    }
}

struct OpenSSHProxyJumpConfiguration: Sendable {
    let host: String
    let port: Int
...
final class RemuxSSHRoot: @unchecked Sendable {
    ...
    private let jumpRoot: RemuxSSHRoot?

    fileprivate init(
        rootChannel: Channel,
        childChannelOpener: RemuxSSHChildChannelOpener,
        jumpRoot: RemuxSSHRoot? = nil
    ) {
        self.rootChannel = rootChannel
        self.childChannelOpener = childChannelOpener
        self.jumpRoot = jumpRoot
    }
    ...
    func close() async {
        do {
            ...
            NSLog("Wissh authenticated SSH root close failed: %@", String(describing: error))
        }
        await jumpRoot?.close()
    }
...
private final class RemuxSSHChannelDataCodec: ChannelDuplexHandler, Sendable {
    ...
    func channelRead(context: ChannelHandlerContext, data: NIOAny) {
        ...
        guard case .channel = channelData.type,
              case .byteBuffer(let bytes) = channelData.data else {
            context.fireErrorCaught(RemuxSSHRootServiceError.unsupportedInboundChannel)
            return
        }
    ...
    func write(
        ...
        context.write(
            wrapOutboundOut(
                SSHChannelData(type: .channel, data: .byteBuffer(unwrapOutboundIn(data)))
            ),
            promise: promise
        )
    }
...
struct RemuxSSHPreparedRoot {
    ...
    func cancelAndCleanup() async {
        ...
        do {
            ...
            NSLog("Wissh prepared SSH connection cleanup failed: %@", String(describing: error))
        }
    }
}
    ...
    func prewarmConnection(
        ...
        Task {
            do {
                ...
                NSLog(
                    "Wissh SSH root prewarm failed for %@@%@:%d (%@): %@",
                    key.username,
                    key.host,
                    key.port,
                    reason,
    ...
    private func scheduleIdleCloseIfNeeded(
        ...
        entry.idleCloseTask = Task {
            do {
                ...
                NSLog("Wissh SSH root idle close timer failed: %@", String(describing: error))
            }
        }
        entries[key] = entry
    }
    ...
    private nonisolated func closeEntryTask(
        ...
        Task {
            do {
                ...
                NSLog("Wissh SSH root pool close failed (%@): %@", reason, String(describing: error))
            }
        }
    }
...
private enum RemuxSSHRootBootstrap {
    static func authenticate(
        ...
        guard let jump = configuration.jump else {
            return try await authenticateDirect(using: configuration, trace: trace)
        }

        let jumpRoot = try await authenticateDirect(
            using: RemuxSSHRootConfiguration(
                host: jump.host,
                port: jump.port,
                authenticationMethod: jump.authenticationMethod,
                hostKeyValidator: jump.hostKeyValidator,
                connectTimeout: jump.connectTimeout,
                authenticationTimeout: jump.authenticationTimeout
            ),
            trace: trace
        )

        do {
            return try await authenticateTarget(
                using: configuration,
                through: jumpRoot,
                trace: trace
            )
    ...
    private static func authenticateTarget(
        using configuration: RemuxSSHRootConfiguration,
        through jumpRoot: RemuxSSHRoot,
        trace: RemuxTransportStartupTrace
    ) async throws -> RemuxSSHRoot {
        ...
        let channel = try await jumpRoot.openDirectTCPIPChannel(
            to: target,
            originatorAddress: originatorAddress,
            trace: trace
        ) { channel in
            ...
            let sshHandler = NIOSSHHandler(
                ...
                inboundChildChannelInitializer: { childChannel, _ in
                    childChannel.eventLoop.makeFailedFuture(
                        RemuxSSHRootServiceError.unsupportedInboundChannel
                    )
                }
            )
            return channel.pipeline.addHandler(RemuxSSHChannelDataCodec()).flatMap {
        ...
        do {
            let handshake = try await channel.pipeline.handler(
                type: RemuxSSHRootHandshakeHandler.self
            ).get()
            try await trace.stage("jump.targetSSHAuthentication") {
                try await handshake.authenticated.get()
            }
            let sshHandler = try await channel.pipeline.handler(
    ...
    private static func authenticateDirect(
        using configuration: RemuxSSHRootConfiguration,
        trace: RemuxTransportStartupTrace
    ) async throws -> RemuxSSHRoot {
