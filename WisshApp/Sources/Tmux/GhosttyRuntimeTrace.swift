...
enum GhosttyRuntimeTrace {
    ...
    static let isEnabled = ProcessInfo.processInfo.environment["WISSH_TRACE_GHOSTTY_IO"] == "1"
    private static let latencyMode = ProcessInfo.processInfo.environment["WISSH_TRACE_LATENCY"]
    ...
    static let diagnosticsEnabled = isEnabled ||
        ProcessInfo.processInfo.environment["WISSH_TRACE_GHOSTTY_DIAGNOSTICS"] == "1"
    static let perfEnabled = ProcessInfo.processInfo.environment["WISSH_TRACE_PERF"] == "1"
    static let tmuxViewportEnabled = ProcessInfo.processInfo.environment["WISSH_TRACE_TMUX_VIEWPORT"] == "1"
    ...
    static func diagnostics(_ message: @autoclosure () -> String) {
        ...
        NSLog("Wissh diag %@", message())
    }

    /// Lightweight perf signpost. Gated on WISSH_TRACE_PERF=1 so it's a true
    ...
    static func perf(_ message: @autoclosure () -> String) {
        ...
        NSLog("Wissh perf t=%llu thread=%@ %@", nowNanos(), threadLabel, message())
    }

    /// Wraps a block, recording its entry thread and elapsed duration when
    /// WISSH_TRACE_PERF=1. Always cheap when disabled; the only cost is one
    ...
    static func perfMeasure<T>(_ label: @autoclosure () -> String, _ body: () -> T) -> T {
        ...
        let result = body()
        NSLog(
            "Wissh perf t=%llu thread=%@ %@ elapsed_ms=%@",
            start,
            entryThread,
            label(),
    ...
    static func latency(_ message: @autoclosure () -> String) {
        ...
        NSLog("Wissh latency t=%llu %@", nowNanos(), resolvedMessage)
    }

    static func tmuxViewport(_ message: @autoclosure () -> String) {
        guard tmuxViewportEnabled else { return }
        NSLog("Wissh tmuxViewport t=%llu %@", nowNanos(), message())
    }
    ...
    static let flowTraceEnabled = perfEnabled || latencyEnabled ||
        ProcessInfo.processInfo.environment["WISSH_TRACE_FLOWS"] == "1"
    ...
    private static func logFlow(
        ...
        NSLog("Wissh flow t=%llu %@", timestamp, parts.joined(separator: " "))
    }
...
final class GhosttyLatencyMarkerAccumulator: @unchecked Sendable {
    private let lock = NSLock()
    private let prefix = "__WISSH_LATENCY_"
