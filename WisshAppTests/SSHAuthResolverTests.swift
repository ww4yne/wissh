...
final class SSHAuthResolverTests: XCTestCase {
    ...
    func testResolvesExplicitProxyJumpIdentityAndUsername() async throws {
        let identity = SSHIdentity(
            name: "Jump password",
            authenticationKind: .password
        )
        let credentialStore = TestSSHCredentialStore(credentials: [
            identity.id: .password("jump-secret"),
        ])
        let resolver = SSHAuthResolver(credentialStore: credentialStore)
        ...
        XCTAssertEqual(auth.identityID, identity.id)
        XCTAssertEqual(auth.username, "jump-user")
        XCTAssertEqual(auth.displayLabel, "Jump password")
        XCTAssertEqual(auth.credential, .password("jump-secret"))
        let loadedIdentityIDs = await credentialStore.loadedIdentityIDs()
        XCTAssertEqual(loadedIdentityIDs, [identity.id])
    }
