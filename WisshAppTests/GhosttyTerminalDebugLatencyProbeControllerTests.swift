...
final class GhosttyTerminalDebugLatencyProbeControllerTests: XCTestCase {
    func testDebugLatencyProbeBuildsInputMarkerWithoutEchoingFullMarker() {
        ...
        XCTAssertEqual(submission?.marker, "__WISSH_LATENCY_abc123__")
        XCTAssertEqual(submission?.text, "printf __WISSH_%s__ LATENCY_abc123\r")
        XCTAssertFalse(submission?.text?.contains("__WISSH_LATENCY_abc123__") ?? true)
    ...
    func testDebugLatencyProbeReadsDelayFromEnvironment() {
        let probe = DebugLatencyProbeCommand.fromEnvironment([
            "WISSH_DEBUG_LATENCY_PROBE": "input",
            "WISSH_DEBUG_LATENCY_PROBE_DELAY_MS": "2500",
        ])
    ...
    func testControllerDoesNotSubmitBeforeDelayIsSatisfied() {
        ...
        XCTAssertEqual(inputs, ["printf __WISSH_%s__ LATENCY_abc\r"])
    }
    ...
    func testRejectedInputProbeCanRetry() {
        ...
        XCTAssertEqual(
            inputs,
            [
                "printf __WISSH_%s__ LATENCY_abc\r",
                "printf __WISSH_%s__ LATENCY_abc\r",
            ]
        )
    }
